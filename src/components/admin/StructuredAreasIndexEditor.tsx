"use client";

import InlineRichTextField from "./InlineRichTextField";

export interface AreasIndexPageContent {
  heroTitle: string;
  heroDescription: string;
  mainHeading: string;
  mainDescription: string;
  whyLocalHeading: string;
  localBenefits: { title: string; description: string }[];
  ctaHeadline: string;
}

export default function StructuredAreasIndexEditor({
  content,
  onChange,
}: {
  content: AreasIndexPageContent;
  onChange: (content: AreasIndexPageContent) => void;
}) {
  // ── Benefits ─────────────────────────────────────────────────────
  function updateBenefit(index: number, field: "title" | "description", value: string) {
    const updated = [...content.localBenefits];
    updated[index] = { ...updated[index], [field]: value };
    onChange({ ...content, localBenefits: updated });
  }
  function addBenefit() {
    onChange({ ...content, localBenefits: [...content.localBenefits, { title: "", description: "" }] });
  }
  function removeBenefit(index: number) {
    onChange({ ...content, localBenefits: content.localBenefits.filter((_, i) => i !== index) });
  }

  return (
    <div className="space-y-8">
      {/* Hero Banner */}
      <Section title="Hero Banner" hint="The main heading (H1) and description shown in the hero section.">
        <label className="block text-xs text-gray-500 mb-1">Page Title (H1)</label>
        <input
          type="text"
          value={content.heroTitle}
          onChange={(e) => onChange({ ...content, heroTitle: e.target.value })}
          className="input-field mb-3"
          placeholder="e.g. Service Areas"
        />
        <label className="block text-xs text-gray-500 mb-1">Hero Description</label>
        <InlineRichTextField
          value={content.heroDescription}
          onChange={(html) => onChange({ ...content, heroDescription: html })}
          rows={3}
          placeholder="Hero description text..."
        />
      </Section>

      {/* Main Content */}
      <Section title="Main Content" hint="Primary content heading and description.">
        <label className="block text-xs text-gray-500 mb-1">Main Heading</label>
        <input
          type="text"
          value={content.mainHeading}
          onChange={(e) => onChange({ ...content, mainHeading: e.target.value })}
          className="input-field mb-3"
          placeholder="Main content heading..."
        />
        <label className="block text-xs text-gray-500 mb-1">Main Description</label>
        <InlineRichTextField
          value={content.mainDescription}
          onChange={(html) => onChange({ ...content, mainDescription: html })}
          rows={3}
          placeholder="Main content description..."
        />
      </Section>

      {/* Why Local */}
      <Section title="Why Local" hint="Heading and benefit cards for the local advantage section.">
        <label className="block text-xs text-gray-500 mb-1">Section Heading</label>
        <input
          type="text"
          value={content.whyLocalHeading}
          onChange={(e) => onChange({ ...content, whyLocalHeading: e.target.value })}
          className="input-field mb-4"
          placeholder="Why local heading..."
        />
        <label className="block text-xs text-gray-500 mb-1">Benefits</label>
        <div className="space-y-3">
          {content.localBenefits.map((benefit, i) => (
            <div key={i} className="bg-gray-800/50 border border-gray-700/50 rounded-lg p-3">
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs text-gray-500">Benefit {i + 1}</span>
                <button
                  type="button"
                  onClick={() => removeBenefit(i)}
                  className="p-1 text-red-400 hover:text-red-300 hover:bg-red-500/10 rounded transition-colors"
                  title="Remove"
                >
                  <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                  </svg>
                </button>
              </div>
              <input
                type="text"
                value={benefit.title}
                onChange={(e) => updateBenefit(i, "title", e.target.value)}
                className="input-field mb-2"
                placeholder="Benefit title..."
              />
              <InlineRichTextField
                value={benefit.description}
                onChange={(html) => updateBenefit(i, "description", html)}
                rows={2}
                placeholder="Benefit description..."
              />
            </div>
          ))}
        </div>
        <button
          type="button"
          onClick={addBenefit}
          className="mt-3 px-3 py-1.5 text-sm text-green-400 border border-green-500/30 rounded-lg hover:bg-green-500/10 transition-colors"
        >
          + Add Benefit
        </button>
      </Section>

      {/* CTA */}
      <Section title="CTA Section" hint="The call-to-action headline at the bottom of the page.">
        <label className="block text-xs text-gray-500 mb-1">CTA Headline</label>
        <input
          type="text"
          value={content.ctaHeadline}
          onChange={(e) => onChange({ ...content, ctaHeadline: e.target.value })}
          className="input-field"
          placeholder="e.g. Need a Roofer in Your Area?"
        />
      </Section>
    </div>
  );
}

function Section({
  title,
  hint,
  children,
}: {
  title: string;
  hint?: string;
  children: React.ReactNode;
}) {
  return (
    <div>
      <h3 className="text-white font-semibold text-sm mb-1">{title}</h3>
      {hint && <p className="text-gray-500 text-xs mb-3">{hint}</p>}
      {children}
    </div>
  );
}
