...
export const SERVICE_BODY_SECTIONS: Record<string, BodySection[]> = {
  ...
  "soffit-repair": [
    ...
    {
      heading: "Soffit vs. Fascia — Which One Is Actually Damaged?",
      html: "<p>Homeowners use the two words interchangeably, but they are different boards doing different jobs, and telling them apart changes what the repair costs. The <strong>fascia</strong> is the vertical board running along the roof edge — it's the flat face your gutters are screwed into, visible from the street. The <strong>soffit</strong> is the horizontal panel tucked underneath, only visible when you stand below the overhang and look up.</p><p>The practical difference: fascia rot is usually visible from the ground, so people catch it early. Soffit rot hides above your head and is typically discovered late — often when a squirrel gets into the attic or paint starts bubbling on the eave. Because gutters hang on the fascia and the soffit tucks in behind it, water that damages one nearly always reaches the other. If you're searching for soffit and fascia repair, assume the inspection needs to cover both; quoting one without checking the other is how contractors end up back at your house in eighteen months.</p>",
    },
    {
      heading: "Repair or Replace? How We Decide on Your Soffit",
      html: "<p>Not every damaged soffit needs full replacement, and we'd rather tell you that up front. <strong>Spot repair makes sense</strong> when damage is confined to one or two panel sections, the fascia behind it is sound, the material is still available to match, and the root cause (usually a gutter problem) can be corrected at the same time. Expect a single-day job in the $300–$1,200 range.</p><p><strong>Full-run replacement is the honest answer</strong> when rot has travelled along the eave, when wood soffit is past year 35 and failing in multiple places, when animals have opened several entry points, or when the existing soffit is unvented and your attic is cooking your shingles from below. On a mid-sized Philadelphia home that generally lands between $1,500 and $3,500, and it's worth doing in one pass rather than chasing failures panel by panel. We put the recommendation in writing with photos of what we found, so you can see the reasoning rather than take our word for it.</p>",
    },
  ],
